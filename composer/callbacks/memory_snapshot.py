...
class MemorySnapshot(Callback):
    ...
    def __init__(
        ...
        if self.remote_file_name:
            _, _, self.remote_path_in_bucket = parse_uri(self.remote_file_name)
        else:
    ...
    def init(self, state: State, logger: Logger) -> None:
        ...
        if model_device.type not in ("cuda", "meta"):
            warnings.warn(
                f"The memory snapshot only works on CUDA devices, but the model is on {model_device.type}."
            )
            self._enabled = False
        else:
    ...
    def batch_start(self, state: State, logger: Logger) -> None:
        if (
            ...
            and self.batches_left_to_skip == 0
        ):
    ...
    def batch_end(self, state: State, logger: Logger) -> None:
        ...
        if state.timestamp.get(self.interval.unit).value == (
            self._start_time.get(self.interval.unit).value + self.interval.value
        ):
    ...
    def stop_record_memory_history(self) -> None:
        log.info("Stopping snapshot record memory history")
    ...
    def export_memory_snapshot(self, state: State, logger: Logger) -> None:
        ...
        filename = os.path.join(
            self.folder_name,
            format_name_with_dist_and_time(
                self.filename, run_name=state.run_name, timestamp=state.timestamp
            ),
        )
        try:
            snapshot_file = f"{filename}.pickle"
            trace_plot_file = f"{filename}.html"
            log.info("Saving memory snapshot files")

            snapshot = torch.cuda.memory._snapshot()

            # No data was recorded - avoids a `ValueError` in `trace_plot`
            if all(len(t) == 0 for t in snapshot["device_traces"]):
                log.info("No allocation is recorded in memory snapshot")
                return
            ...
            if self.remote_path_in_bucket is not None:
                remote_files = [snapshot_file, trace_plot_file]
                for f in remote_files:
                    remote_file_name = os.path.join(
                        self.remote_path_in_bucket, os.path.basename(f)
                    ).lstrip("/")
                    log.info(
                        f"Uploading memory snapshot to remote: {remote_file_name} from {f}"
                    )
                    try:
                        logger.upload_file(
