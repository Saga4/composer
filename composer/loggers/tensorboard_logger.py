...
class TensorboardLogger(LoggerDestination):
    ...
    def __init__(
        ...
        self._cached_rank: Optional[int] = None
    ...
    def _flush(self, logger: Logger):
        ...
        if self.rank_zero_only and self._get_rank() != 0:
            return
        ...
        logger.upload_file(
            remote_file_name=f"tensorboard_logs/{{run_name}}/{event_file_name}-{self._get_rank()}",
            file_path=file_path,
            overwrite=True,
        )
    ...
    def _get_rank(self):
        if self._cached_rank is None:
            self._cached_rank = dist.get_global_rank()
