...
from composer.trainer._patch_pytorch import (
    patch_pytorch,
    patch_unshard_for_automicrobatching,
)
...
def _get_default_scheduler_frequency(
    ...
    has_pytorch_scheduler = any(
        isinstance(scheduler, LRScheduler) for scheduler in ensure_tuple(schedulers)
    )
...
def _set_evaluator_interval_and_subset_num_batches(
    ...
    for evaluator in evaluators:
        ...
        if (
            isinstance(eval_dataloader, collections.abc.Sized)
            and evaluator.subset_num_batches == -1
        ):
            try:
...
def _is_auto_microbatching(
    device_train_microbatch_size: Optional[Union[int, float, str]], device: Device
):
...
def _get_initial_device_train_microbatch_size(
    ...
    """
    if device_train_microbatch_size is not None and not auto_microbatching:
        if isinstance(device_train_microbatch_size, (int, float)):
            return device_train_microbatch_size
        raise ValueError("device_train_microbatch_size must be an int or `auto`")

    # Handling auto_microbatching or None cases
...
def _update_num_consecutive_thrashes(
    state: State, num_consecutive_thrashes: int, num_alloc_retries: int
):
    ...
    if cur_num_alloc_retries - num_alloc_retries > 0:
        ...
        log.info(
            "Found new alloc retries this batch: "
            + str(num_alloc_retries)
            + " to "
            + str(cur_num_alloc_retries)
        )
    else:
...
def _create_sync_hook(state: State):
    ...
    def sync_hook(*args):
        ...
        all_ranks_finished_tensor = state.device.tensor_to_device(
            torch.tensor([0], dtype=torch.uint8)
        )
...
def _readd_fsdp_sync_hooks(fsdp_modules: dict[str, torch.nn.Module], sync_hook):
    ...
    for module in fsdp_modules.values():
        if isinstance(module, FullyShardedDataParallel):
            automicrobatch_fsdp_hook_handles.append(
...
def _validate_evaluator(evaluator: Evaluator, device: Device):
    ...
    if evaluator.auto_microbatching and hasattr(
    ...
    if (
        ...
        and evaluator.dataloader.seq_parallel_world_size > 1
        and abs(  # type: ignore
...
class Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            and hasattr(train_dataloader, "seq_parallel_world_size")
        ):
            raise ValueError(
                '`device_train_microbatch_size="auto"` is not compatible with sequence parallelism.'
            )
        if (
            ...
            and train_dataloader.seq_parallel_world_size > 1
            and abs(  # type: ignore
                device_train_microbatch_size * train_dataloader.seq_parallel_world_size
                - 1,  # type: ignore
            )
            > 1e-4
        ):
        ...
        # Distributed
        if parallelism_config is not None and not isinstance(
            ...
            if "fsdp" in parallelism_config and parallelism_config["fsdp"] is not None:
                if isinstance(parallelism_config["fsdp"], FSDPConfig):
                    ...
                    parallelism_config_args["fsdp"] = FSDPConfig(
            ...
            parallelism_config = (
                ...
                if len(parallelism_config_args) > 0
                else None
            )
        ...
        _checkpoint_savers = [
            cb for cb in self.state.callbacks if isinstance(cb, CheckpointSaver)
        ]
        if len(_checkpoint_savers) >= 1:
            if len(_checkpoint_savers) > 1:
                log.info(
                    "Multiple CheckpointSaver provided as callbacks. Using the first one as reference."
                )
            ...
            if self._checkpoint_saver.latest_filename is None:
                ...
                self._checkpoint_saver.latest_filename.filename != save_latest_filename
            ):
                save_latest_filename = str(
                    self._checkpoint_saver.latest_filename.filename
                )
                log.info(f"Using {save_latest_filename} as latest_filename.")

            if self._checkpoint_saver.latest_remote_file_name is not None:
                latest_remote_file_name = str(
                    self._checkpoint_saver.latest_remote_file_name.filename
                )
        ...
        if save_folder is not None:
            ...
            if mlflow_logger is not None:
                ...
                if latest_remote_file_name is not None:
                    latest_remote_file_name = partial_format(
                        latest_remote_file_name, **mlflow_format_kwargs
                    )
        ...
        if (
            ...
            and self.state.fsdp_config.auto_wrap
                        ...
                        with tempfile.TemporaryDirectory() as temp_dir:
                            ...
                            try:
                                ar_object_store.download_object(
                                    symlink_file_name, local_symlink_file
                                )
                                with open(local_symlink_file, "r") as f:
                                    real_path = f.read()
                                    log.debug(
                                        f"Read path {real_path} from symlink file"
                                    )
        ...
        if (
            ...
            and self.state.fsdp_config.auto_wrap
            and self.state.load_monolith_rank0_only
        ):
            # Init with globally fixed seed so all HSDP replicas have the same initial weights
            with reproducibility.seed_context(self.state.rank_zero_seed):
                self.state.automicrobatch_fsdp_hook_handles, self.state.fsdp_modules = (
        ...
        if (
            self.state.timestamp.iteration == 0
            and self.state.timestamp.token_in_iteration == 0
            and self.state.timestamp.epoch_in_iteration == 0
        ):
    ...
    def _try_checkpoint_download(
        ...
        if (
            ...
            and self._checkpoint_saver.remote_uploader is not None
        ):
            remote_destination.append(
                self._checkpoint_saver.remote_uploader.remote_backend
            )
    ...
    def fit(
        ...
        *   If ``reset_time`` is False, then :meth:`.fit` will train for the amount of time specified by
        ...
        if (
            type(self.state.optimizers[0]) == torch.optim.SGD
            and version.parse(torch.__version__) >= version.parse("2.4.0")
        ...
        if device_train_microbatch_size is not None:
            ...
            if (
                self.state.auto_microbatching
                and self._train_data_spec is not None
                and hasattr(
                    self._train_data_spec,
                    "seq_parallel_world_size",
                )
    ...
    def _accumulate_time_across_ranks(
        ...
        if isinstance(num_samples, float):
            ...
            if torch.any(
                torch.abs(sample_token_tensor_int - sample_token_tensor) > 1e-4
            ):
                raise ValueError(
                    "The sums of samples and tokens across ranks should each be integers."
                )
    ...
    def _train_loop(self) -> None:
        ...
        while self.state.timestamp < self.state.max_duration:
            ...
            for batch_idx, self.state.batch in enumerate(
                ...
                self.state.batch = self._train_data_spec.batch_transforms(
                    self.state.batch
                )
                ...
                if self.state.timestamp >= self.state.max_duration or (
                    self.state._iteration_length is not None
                    and self.state.timestamp.token_in_iteration.unit
                    == self.state._iteration_length.unit
                    and self.state.timestamp.token_in_iteration
            ...
            if (
                ...
                if (
                    self.state._iteration_length is not None
                    and self.state.timestamp.epoch_in_iteration.unit
                    == self.state._iteration_length.unit
        ...
        while True:
            ...
            if self.state.auto_microbatching:
                ...
                if found_cuda_oom == 1:
                    ...
                    if (
                        self.state.fsdp_enabled
                        and len(self.state.automicrobatch_fsdp_hook_handles) == 0
                    ):
                        self.state.automicrobatch_fsdp_hook_handles = (
                            _readd_fsdp_sync_hooks(
            ...
            if (
                ...
                > 0
                and self.num_consecutive_non_OOM_batches >= 3
            ):
        ...
        with context():
            ...
            if self.accumulate_train_batch_on_tokens:
                current_batch_size = sum(
                    [
                        self._train_data_spec.get_num_tokens_in_batch(
                            b, token_type="loss_generating"
                        )
                ...
                current_batch_size = sum(
                    [
                        self._train_data_spec.get_num_samples_in_batch(b)
                        for b in microbatches
                    ]
                )
            # Average the current batch size across ranks, to ensure each rank contributes appropriately
            current_batch_size = self.state.device.tensor_to_device(
                torch.tensor(current_batch_size)
            )
            ...
            for microbatch_idx, self.state.batch in enumerate(microbatches):
                ...
                self.state.batch = self._train_data_spec.microbatch_transforms(
                    self.state.batch
                )
    ...
    def _train_microbatch(
        ...
        if self.accumulate_train_batch_on_tokens:
            ...
            microbatch_size = self._train_data_spec.get_num_samples_in_batch(
                self.state.batch
            )
        ...
        with sync_context:
            ...
            for k, loss in microbatch_loss_dict.items():
                microbatch_loss_dict[k] = loss.detach().clone().mean() * (
                    microbatch_size / current_batch_size
                )
            ...
            if (
                self.state.train_metrics is not None  # pyright: ignore[reportUnnecessaryComparison]
    ...
    def _eval_loop(
        ...
        with torch.no_grad(), model_eval_mode(self.state.model):
            ...
            if isinstance(dist_sampler, DistributedSampler) and isinstance(
            ...
            for self.state.batch in self._iter_dataloader(TrainerMode.EVAL):
                ...
                if (
                    ...
                    if abs(batch_num_samples - batch_num_samples_tensor.item()) > 1e-4:
                        raise ValueError(
                            "Number of samples in a batch should be an integer."
                        )
                    last_batch = (
                ...
                while True:
                    ...
                    try:
                        ...
                        for i, self.state.batch in enumerate(microbatches):
                            ...
                            self.state.batch = data_spec.microbatch_transforms(
                                self.state.batch
                            )
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    if num_samples_in_microbatch == 1 or hasattr(
                                        data_spec, "seq_parallel_world_size"
                                    ):
                                        skip_metric_update = True
                                    # Remove padded samples from batch
                                    else:
                                        if not isinstance(
                                            num_samples_in_microbatch, int
                                        ):
                                            raise ValueError(
                                                "Number of samples in a batch should be an integer."
                                            )
    ...
    def _iter_dataloader(self, trainer_mode: TrainerMode):
        ...
        iter_finished = self.state.device.tensor_to_device(
